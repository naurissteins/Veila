...
impl SceneMetrics {
    pub(super) fn from_frame(width: i32, height: i32) -> Self {
        ...
        let input_height = (((height as f32) * 0.08) as i32).clamp(52, 64);
        let avatar_size = (width.min(height) / 6).clamp(88, 120);
...
pub(super) fn role_anchors(
    ...
    let hero_y = centered_role_top(frame_height, hero_height, 0.39);
    let mut auth_y = centered_role_top(frame_height, auth_height, 0.64);
    let footer_y = frame_height - footer_height - 48;
    let minimum_gap = 18;

    if auth_y < hero_y + hero_height + minimum_gap {
