...
impl SceneMetrics {
    ...
    pub fn new(
        ...
        let auto_avatar_size_max = 160.max(width.min(height) / 4);
        let configured_avatar_size_max = 56.max(width.min(height).saturating_mul(4) / 5);
        ...
        let avatar_size = configured_avatar_size.map_or_else(
            || {
                (width.min(height) / 7)
                    .clamp(84, 108)
                    .clamp(56, auto_avatar_size_max)
            },
            |size| size.clamp(56, configured_avatar_size_max),
        );
        Self {
            center_x: width / 2,
