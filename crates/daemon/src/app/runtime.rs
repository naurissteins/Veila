...
/// Promotes a pre-warmed standby curtain to the active locked state.
pub(super) async fn activate_lock_via_standby(
    session_proxy: &logind::SessionProxy<'_>,
    state: &mut LockState,
    standby: Child,
    standby_control_socket: &std::path::Path,
) -> Result<LockActivation> {
    *state = LockState::Locking;
    ...
    let notify_str = notify_path.display().to_string();
    let auth_str = auth_socket_path.display().to_string();

    if let Err(error) =
        process::send_curtain_lock_now(standby_control_socket, notify_str, auth_str).await
    {
        *state = LockState::Unlocked;
        let _ = std::fs::remove_file(&notify_path);
        let _ = std::fs::remove_file(&auth_socket_path);
        process::force_stop_curtain(standby).await?;
        return Err(error).context("failed to send LockNow to standby curtain");
    }

    let mut child = standby;
    let ready_result = tokio::select! {
        ready = timeout(Duration::from_secs(5), notify_listener.accept()) => ReadyResult::Ready(ready),
        status = child.wait() => ReadyResult::Exited(
            status.context("failed while waiting for standby curtain exit before readiness")?
        ),
    };
    let _ = std::fs::remove_file(&notify_path);
    ...
    match ready_result {
        ReadyResult::Ready(Ok(Ok((_stream, _addr)))) => {
            *state = LockState::Locked;
            update_locked_hint(session_proxy, true).await;
            tracing::info!("standby curtain ready; session considered locked");
            Ok(LockActivation {
                curtain: child,
                auth_listener,
                auth_socket_path,
                control_socket_path: standby_control_socket.to_path_buf(),
                auth_results,
                auth_sender,
            })
        }
        ReadyResult::Ready(Ok(Err(error))) => {
            *state = LockState::Unlocked;
            ...
            process::force_stop_curtain(child).await?;
            update_locked_hint(session_proxy, false).await;
            Err(error).context("failed while waiting for standby curtain readiness")
        }
        ReadyResult::Ready(Err(_)) => {
            *state = LockState::Unlocked;
            let _ = std::fs::remove_file(&auth_socket_path);
            process::force_stop_curtain(child).await?;
            update_locked_hint(session_proxy, false).await;
            Err(anyhow!("timed out waiting for standby curtain readiness"))
        }
        ReadyResult::Exited(status) => {
            *state = LockState::Unlocked;
            let _ = std::fs::remove_file(&auth_socket_path);
            update_locked_hint(session_proxy, false).await;
            Err(anyhow!(
                "standby curtain exited before readiness with status {status}"
            ))
        }
    }
}
